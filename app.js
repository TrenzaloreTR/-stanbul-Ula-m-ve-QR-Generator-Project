...
themeToggle.addEventListener('click', () => { document.body.classList.toggle('light-mode'); themeToggle.innerText = document.body.classList.contains('light-mode') ? 'Karanlık Mod' : 'Gündüz Modu'; });
...
const baseMaps = { "Sokak Haritası": googleStreets, "Uydu": googleSatellite, "Hibrit": googleHybrid };
const overlayMaps = {};

document.body.classList.add('hide-labels');
map.on('zoomend', () => { if (map.getZoom() >= 13) document.body.classList.remove('hide-labels'); else document.body.classList.add('hide-labels'); });
...
for (const [lineName, stations] of Object.entries(transitData)) {
  ...
  let lineGroup = L.layerGroup();
  
  L.polyline(coords, {color: color, weight: 5, opacity: 0.9}).addTo(lineGroup);
  
  stations.forEach(s => {
    L.circleMarker([s.lat, s.lng], { radius: 5, fillColor: "#fff", color: color, weight: 2, fillOpacity: 1 })
      .addTo(lineGroup)
      .bindTooltip(s.name, { permanent: true, direction: 'right', className: 'station-label' });
  });
  
  lineGroup.addTo(map);
  overlayMaps[lineName] = lineGroup; // Menüye eklenecek katmanlar
}

// Filtreleme menüsünü haritaya ekle
...
if ('geolocation' in navigator) {
  navigator.geolocation.watchPosition((pos) => {
    ...
    if(!userMarker) { userMarker = L.marker([lat, lng], {icon: gpsIcon}).addTo(map); map.setView([lat, lng], 14); }
